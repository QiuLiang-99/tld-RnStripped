// https://raw.githubusercontent.com/ShieldHeart1/RelentlessNight/master/Gameplay/CarcassMoving.cs
...
namespace ttrRnStripped
{
	internal class CarcassMoving : MonoBehaviour
	{
		...
		internal class Panel_BodyHarvest_CanEnable
		{
			...
			{
				if (!Settings.options.carcassMovingEnabled) return;
		...
		internal class Panel_BodyHarvest_Enable
		{
			...
			{
				if ( !enable || !__instance.CanEnable(bh)) return;
		...
		internal class LoadScene_Activate
		{
			private static void Postfix()
			{
				if (!Settings.options.carcassMovingEnabled || !isCarryingCarcass || carcassObj == null) return;
		...
		internal class GameAudioManager_PlayGUIError_Pre
		{
			...
			{
				if (!Settings.options.carcassMovingEnabled) return true;
		...
		internal class PlayerManager_EquipItem
		{
			private static bool Prefix()
			{
				if (!Settings.options.carcassMovingEnabled || !isCarryingCarcass) return true;
